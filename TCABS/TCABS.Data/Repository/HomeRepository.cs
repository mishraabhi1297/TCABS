...
namespace TCABS.Data.Repository
{
    public class HomeRepository
    {
        ...
        public HomeRepository(IConnectionProvider connection, IUnitOfWork unitOfWork)
        {
            _transaction = unitOfWork;
            _connectionProvider = connection;
        }

        public async Task<IEnumerable<UnitAssociation>> GetUnitsForUserAsync(int? id)
        {
            try
            {
                using (var connection = _connectionProvider.Create())
                {
                    return await connection.QueryAsync<UnitAssociation>("dbig5_admin.READ_UNITS_FOR_USER_VIASQLDEV",
                        new { pUserID = id }, commandType: CommandType.StoredProcedure);
                }
            }
            catch (Exception ex)
            {
